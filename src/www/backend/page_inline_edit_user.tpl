...
<body class="[:$DOJO_THEME:]">
    <h1 class="heading align-center viewport-800">[:$MODEL.language->getMessage('msg.editUser'):]</h1>
   <div id="[:$FORM.name|escape:'html':]" class="align-center viewport-800" name="[:$FORM.name|escape:'html':]" dojoType="dijit.form.Form" 
        ...
        <label for="form_groupId">[:$LANG->getMessage('msg.user.group'):]:</label>
        <select name="groupId" id="form_groupId" style="width: 150px">
            <option value="1" [:if $MODEL.form.groupId==1:]selected="selected"[:/if:]>[:$LANG->getMessage('msg.user.group.admin'):]</option>
            <option value="2" [:if $MODEL.form.groupId==2:]selected="selected"[:/if:]>[:$LANG->getMessage('msg.user.group.shopOwner'):]</option>
        </select>
        <br />
        ...
        <input dojoType="dijit.form.ValidationTextBox" id="form_title" style="width: 20%" type="text" name="title" value="[:$MODEL.form.title|escape:'html':]" />
        <br />
        
        <label for="form_fullname">[:$LANG->getMessage('msg.user.fullname'):]:</label>
        <input dojoType="dijit.form.ValidationTextBox" id="form_fullname" style="width: 50%" type="text" name="fullname" value="[:$MODEL.form.fullname|escape:'html':]" />
        <br />
        ...
        <input id="form_email" style="width: 50%"  required="true" missingMessage="[:$LANG->getMessage('error.emptyEmail'):]"
            ...
            invalidMessage="[:$LANG->getMessage('error.invalidEmail'):]"
